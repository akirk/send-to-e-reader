...
class Abilities {
	...
	public function list_posts( $input = null ) {
		...
		$meta_query = Send_To_E_Reader::get_sent_status_meta_query( $sent_status );
		if ( ! empty( $meta_query ) ) {
		...
		foreach ( $query->get_posts() as $post ) {
			if ( ! $this->plugin->current_user_can_send( $post ) ) {
				continue;
			}
	...
	private function get_posts_from_input( array $input ) {
		...
		foreach ( $post_ids as $post_id ) {
			...
			// The same per-post check the admin, Friends and Post Collection
			// send paths use, so the send_to_e_reader_user_can_send filter also
			// applies to abilities.
			if ( ! $this->plugin->current_user_can_send( $post ) ) {
				return new \WP_Error(
					'cannot-send-post',
					sprintf(
						/* translators: %d is a post ID. */
						__( 'You are not allowed to send post ID %d to an e-reader.', 'send-to-e-reader' ),
						$post_id
					)
				);
			}
